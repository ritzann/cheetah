...
def _kde_joint_pdf_3d(
    ...
    """
    Compute the joint probability distribution function for three input tensors based on
    the number of histogram bins.

    :param kernel_values1: shape :math:`(B, N, N_{bins})`.
    :param kernel_values2: shape :math:`(B, N, N_{bins})`.
    :param kernel_values3: shape :math:`(B, N, N_{bins})`.
    :param epsilon: A scalar, for numerical stability. Default: 1e-10.
    :return: Kernel density estimation of the joint probability distribution function of
        shape :math:`(B, N_{bins}, N_{bins}, N_{bins})`.
    """
    ...
    B, N, N_bins = kernel_values1.shape

    # shape kernels
    k1 = kernel_values1.unsqueeze(-1).unsqueeze(-1)   # (B, N, N_bins, 1, 1)
    k2 = kernel_values2.unsqueeze(-2).unsqueeze(-1)   # (B, N, 1, N_bins, 1)
    k3 = kernel_values3.unsqueeze(-2).unsqueeze(-2)   # (B, N, 1, 1, N_bins)
    # element-wise multiply then sum over N (data points)
    joint_kernel_values = k1 * k2 * k3        # (B, N, N_bins, N_bins, N_bins)
    joint_kernel_sum = joint_kernel_values.sum(dim=1)  # (B, N_bins, N_bins, N_bins)

    normalization = joint_kernel_sum.sum(dim=(-3, -2, -1), keepdim=True) + epsilon
    pdf = joint_kernel_sum / normalization
...
def kde_histogram_3d(
    ...
    """
    Estimate the 3D histogram of the input tensor.
    ...
    :param x1: Input tensor to compute the histogram with shape :math:`(B, D1)`.
    :param x2: Input tensor to compute the histogram with shape :math:`(B, D2)`.
    :param x3: Input tensor to compute the histogram with shape :math:`(B, D3)`.
    :param bins: Bin coordinates.
    :param bandwidth: Gaussian smoothing factor with shape shape `(1,)`.
    :param weights: Weights of the input tensor of shape :math:`(B, N)`.
    :param epsilon: A scalar, for numerical stability. Default: 1e-10.
    :return: Computed histogram of shape :math:`(B, N_{bins}, N_{bins}, N_{bins})`.
    ...
    Examples:
        ...
        >>> x3 = torch.rand(2, 32)
        >>> bins = torch.torch.linspace(0, 255, 128)
        >>> hist = kde_histogram_3d(x1, x2, x3, bins, bandwidth=torch.tensor(0.9))
        >>> hist.shape
    ...
    joint_pdf = _kde_joint_pdf_3d(kernel_values1, 
                                  kernel_values2, 
                                  kernel_values3, 
                                  epsilon=epsilon)
